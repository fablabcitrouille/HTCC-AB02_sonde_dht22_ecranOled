...
#define USER_BUTTON_PIN GPIO6 // Remplacez par la broche correcte pour le bouton USER
...
//déclarer ces variables globales pour les utiliser dans les fonctions
...
void setup() {
  ...
  // Afficher le nom du fichier sur l'écran OLED
  display.clear();
  Serial.println(__FILE__);
  display.drawString(0, 0, (__FILE__));
  display.display();
...
void loop() {
  ...
  if (digitalRead(USER_BUTTON_PIN) == LOW) { // Bouton pressé
    display.clear();
    display.drawString(0, 0, "Informations:");
    display.drawString(0, 10, "Temp: 25C");
    display.drawString(0, 20, "Hum: 60%");
    display.display();
  } else { // Bouton relâché
    display.clear();
    display.display();
